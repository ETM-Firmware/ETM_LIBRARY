...
; _branchPortA:	
	.ifdef __PORTA_ACTIVE
		AND             LATA, WREG		; Note that the Mask must be stored in W0	
	.else	
		NOP
	.endif
		RETURN
...
; _branchPortB:
	.ifdef __PORTB_ACTIVE
		AND             LATB, WREG		; Note that the Mask must be stored in W0
	.else
		NOP
	.endif
		RETURN
; _branchPortC:
	.ifdef __PORTC_ACTIVE
		AND             LATC, WREG		; Note that the Mask must be stored in W0	
	.else
		NOP
	.endif
		RETURN
; _branchPortD:
	.ifdef __PORTD_ACTIVE
		AND             LATD, WREG		; Note that the Mask must be stored in W0	
	.else
		NOP
	.endif
		RETURN
; _branchPortE:
	.ifdef __PORTE_ACTIVE
		AND             LATE, WREG		; Note that the Mask must be stored in W0	
	.else	
		NOP
	.endif
		RETURN
; _branchPortF:
	.ifdef __PORTF_ACTIVE
		AND             LATF, WREG		; Note that the Mask must be stored in W0	
	.else
		NOP
	.endif
		RETURN
; _branchPortG:
	.ifdef __PORTG_ACTIVE
		AND             LATG, WREG		; Note that the Mask must be stored in W0	
	.else
		NOP
	.endif
		RETURN
